...
def test_candidate_allows_read_induced_ctime_change_during_hash(tmp_path, monkeypatch):
    """zfuse may advance ctime while a regular file is read for SHA256."""
    f = tmp_path / "candidate.txt"
    content = b"DATA" * 1024
    f.write_bytes(content)
    h = hashlib.sha256(content).hexdigest()
...
def test_candidate_rejects_mtime_change_during_hash(tmp_path, monkeypatch):
    f = tmp_path / "candidate.txt"
    content = b"DATA" * 1024
    f.write_bytes(content)
    h = hashlib.sha256(content).hexdigest()
    ...
    def mock_fstat(fd):
        ...
        if fstat_call_count > 1:
            class FakeStat:
                ...
                st_ino = real_st.st_ino
                st_size = real_st.st_size
                st_mtime_ns = getattr(real_st, "st_mtime_ns", int(real_st.st_mtime * 1e9)) + 1000
                st_ctime_ns = getattr(real_st, "st_ctime_ns", int(real_st.st_ctime * 1e9))
                st_mtime = real_st.st_mtime + 0.001
                st_ctime = real_st.st_ctime
            return FakeStat()
        return real_st
