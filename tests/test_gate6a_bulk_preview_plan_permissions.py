...
def _seed_active_entry(client: TestClient) -> int:
    ...
    payload = b"gate6a-restore-authority"

    with service.SessionLocal() as session:
        entry = QuarantineEntry(
            original_path=str(data / "restore.txt"),
            quarantine_path=str(trash / "pending-restore.txt"),
            state="active",
            tx_phase="active",
        ...
        public_view = trash / f"restore.q-{entry.id}.txt"
...
def test_restore_bulk_plan_requires_allow_mutation(tmp_path: Path) -> None:
    client = _setup_client(tmp_path, allow_mutation=False)
