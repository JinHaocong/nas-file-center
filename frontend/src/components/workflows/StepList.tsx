...
export const StepList: React.FC<StepListProps> = ({ steps, mode, readOnly = false, onChange }) => {
  ...
  const rawMenuItems: Array<{ key: WorkflowStep['type']; icon: React.ReactNode; label: string }> =
    mode === 'file'
      ? [
          { key: 'scan', icon: <FolderOpenOutlined />, label: '扫描根目录 (Scan)' },
          { key: 'filter', icon: <FilterOutlined />, label: '条件过滤 (Filter)' },
          { key: 'rename', icon: <EditOutlined />, label: '字面量重命名 (Rename)' },
          { key: 'move', icon: <FolderOutlined />, label: '路径移动 (Move)' },
          { key: 'touch', icon: <ClockCircleOutlined />, label: '刷新时间戳 (Touch)' },
          { key: 'quarantine', icon: <SafetyCertificateOutlined />, label: '隔离归档 (Quarantine)' },
        ]
      : mode === 'organizer'
        ? [
            { key: 'scan', icon: <FolderOpenOutlined />, label: '扫描根目录 (Scan)' },
            { key: 'organize', icon: <AppstoreOutlined />, label: '目录整理方案 (Organize)' },
          ]
        : [];

  const menuItems: MenuProps['items'] = rawMenuItems.map((item) => ({
    key: item.key, icon: item.icon, label: item.label,
    disabled: !allowedInsertions.includes(item.key), onClick: () => handleAddStep(item.key),
  }));

  const handleStepChange = (index: number, updated: WorkflowStep) => { const nextSteps = [...steps]; nextSteps[index] = updated; onChange(nextSteps); };
  const handleMoveUp = (index: number) => { if (index === 0) return; const nextSteps = [...steps]; [nextSteps[index - 1], nextSteps[index]] = [nextSteps[index], nextSteps[index - 1]]; onChange(nextSteps); };
  const handleMoveDown = (index: number) => { if (index >= steps.length - 1) return; const nextSteps = [...steps]; [nextSteps[index + 1], nextSteps[index]] = [nextSteps[index], nextSteps[index + 1]]; onChange(nextSteps); };
  const handleDelete = (index: number) => onChange(steps.filter((_, i) => i !== index));
