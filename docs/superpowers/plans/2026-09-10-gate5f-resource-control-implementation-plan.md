...
- Produces: Enhanced `claim_next_job()` with canonical two-phase protocol:
  - Phase A (outside write transaction):
    - read current policy
    - validate snapshot
    - capture FULL fingerprint (`resource_policy_row_fingerprint`)
    - pre-resolve timezone if required
    - no write lock, no authoritative lease check
  - Phase B (`BEGIN IMMEDIATE` write transaction):
    - fresh transaction-time `claim_now = utcnow()`
    - fresh Worker lease assertion: `assert_active_worker_lease(session, worker_id, now=claim_now)`
    - re-read policy row and compute FULL fingerprint
    - fingerprint mismatch → rollback and retry Phase A
    - fingerprint match:
      - evaluate active window against `claim_now` + pre-resolved timezone object
      - choose eligible job according to admission and priority
      - mark RUNNING with `started_at=claim_now, heartbeat_at=claim_now`
      - COMMIT
  - 3 unstable attempts (bounded retry exhaustion fallback):
    - `BEGIN IMMEDIATE`
    - fresh lease assertion: `assert_active_worker_lease(session, worker_id, now=fallback_now)`
    - claim eligible NON-resource job only (never use stale policy or claim resource jobs)
    - resource jobs remain queued without fabricating paused/failed states
