# Gate5-G / G7 Transactional Mutation Implementation Plan (v1.1.2)
...
6. **Gate3 Full Physical Authority**: Candidate qualification consumes full Gate3 frozen evidence (`dev`, `ino`, `size`, `mtime_ns`, `ctime_ns`, and `content_hash`). Both `mtime_ns` and `ctime_ns` must be strictly verified pre-hash and post-hash across 1MiB SHA256 chunk streaming. Candidate mismatch enters `conflict`.
7. **Durable & Short Committed Lease Fence**:
   ```python
   with session_factory() as session:
       # BEGIN IMMEDIATE
       # assert owner == worker_id and not expired
       # renew acquired_at = utcnow()
       # COMMIT
   # No SQLite write transaction held here
   execute EXACTLY ONE payload-affecting filesystem syscall
   ```
   Zero SQLite write lock held across filesystem/FUSE calls.
8. **Capability Routing & Probe Precision**:
   - Probe target filesystem directly using `dir_fd` of target directory via `safe_open_parent_fd` to avoid probing target's parent.
...
9. **Restore Finalization Protocol**: Restore does NOT finalize to `state='restored'` upon original link alone. It finalizes ONLY after public quarantine view is retired into the restore attempt slot. If view is foreign/unknown, preserve in place and enter `conflict`.
10. **Real NAS Path Isolation**:
...
12. **TDD Discipline**: Every Task must end GREEN against its focused regressions before committing.
...
| Component / File | Baseline Reality (`c32d077`) | Role in Implementation Plan v1.1.2 |
| :--- | :--- | :--- |
...
| [`app/tasks/handlers.py`](file:///Users/Kerwin/MyProject/nas-file-center/app/tasks/handlers.py#L510-L570) | `gather_reconcile_evidence` & `_validate_evidence` define Gate3 frozen physical identity check (including `mtime_ns` and `ctime_ns`). | Reused as the authoritative qualification model. |
...
| [`app/tasks/recovery.py`](file:///Users/Kerwin/MyProject/nas-file-center/app/tasks/recovery.py#L29-L60) | `assert_active_worker_lease()` checks `TaskLock` row 1 with 30s timeout. | Extended with `renew_and_assert_worker_lease(session_factory, worker_id)`. |
...
### 2.1 Complete Gate3 Authority (Including `ctime_ns`)
...
Candidate qualification executes strictly relative to an opened file descriptor without pathname re-open races:
```python
def qualify_candidate_anchor_fd(
    ...
    if expected_mtime_ns is not None and before_mtime_ns != expected_mtime_ns:
        return False
    if expected_ctime_ns is not None and before_ctime_ns != expected_ctime_ns:
        return False
...
### 3.1 Protocol Specification & Interface
```python
...
## 4. Capability Routing API & Target Probing
...
def resolve_mutation_capability(
    ...
    Probes target_dir directly using its descriptor to avoid probing the parent directory.
    """
    try:
    ...
    # Open target directory safely to get its dir_fd for accurate probing
    try:
        target_dfd = safe_open_parent_fd(Path(target_dir) / "dummy")
    except Exception:
        return MutationCapability.UNSUPPORTED
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **1** | `preparing`, `preparing` | No attempt dir or no anchor in `attempt-<G>` | Stat attempt dir & anchor; verify absent. | Yes ($G+1$) if retrying | `preparing` / `preparing` (or `conflict` if source missing) | NO payload mutation. Create `attempt-<G+1>` dir only. | Committed fence before `os.mkdir` | Retry or fail item |
| **2** | `preparing`, `preparing` | Candidate anchor exists in `attempt-<G>/anchor` | `open(O_RDONLY\|O_NOFOLLOW)`, run `qualify_candidate_anchor_fd()` against Gate3 full identity | No | If valid: `authoritative_anchored` / `preparing`. **If invalid: `conflict` / `conflict`** | Zero unlink. Zero new gen retry. If valid: DB update only. | Committed fence before DB promotion | Progress if valid; `status='failed'` if invalid |
...
| **10**| `restoring`, `restoring` | Original destination absent; anchor valid | Stat original path (absent); stat anchor (valid). | No | **`restoring` / `restoring`** (Do NOT mark restored yet) | YES: `os.link(anchor, original_path)`. | Committed fence before `os.link` | Progress restore to view capture |
| **11**| `restoring`, `restoring` | Original destination present and matches anchor dev/ino | Stat original path & anchor; verify matching dev/ino. Check public view. | No | If view captured expected: **`restored` / `restored`**. If view foreign: **`conflict` / `conflict`**. | If view expected present: `os.rename(public_view, restore_slot)`. | Committed fence before view retirement | `status='succeeded'` or conflict |
...
- Produces: `renew_and_assert_worker_lease(session_factory, worker_id, timeout_seconds) -> None`.

- [ ] **Step 1: Write failing tests for committed lease fence**
```python
# tests/test_gate5g_g7_task2_lease_fence.py
def test_lease_fence_renewal_visible_across_sessions(session_factory):
...
def test_lease_fence_zero_open_transaction_during_syscall(session_factory, db_engine):
    # Call renew_and_assert_worker_lease(session_factory, "worker-1")
    # Assert zero open transaction on db_engine
    pass

def test_lease_fence_rejects_stale_worker_post_takeover(session_factory):
...
Implement committed fence with `session_factory()`, `BEGIN IMMEDIATE`, and immediate commit.
...
### Task 3: Gate3 Full Authority Qualification (Including `ctime_ns`)
...
- Produces: `qualify_candidate_anchor_fd(fd, dev, ino, size, sha256, mtime_ns, ctime_ns) -> bool`.

- [ ] **Step 1: Write failing tests for candidate qualification including ctime**
```python
...
def test_qualify_candidate_anchor_fd_success(tmp_path):
    ...
    with open(f, "rb") as fp:
        assert qualify_candidate_anchor_fd(fp.fileno(), st.st_dev, st.st_ino, len(content), h, mtime_ns, ctime_ns) is True

def test_candidate_rejects_expected_ctime_mismatch(tmp_path):
    f = tmp_path / "candidate.txt"
    content = b"DATA"
    f.write_bytes(content)
    h = hashlib.sha256(content).hexdigest()
    ...
    ctime_ns = getattr(st, "st_ctime_ns", int(st.st_ctime * 1e9))
    with open(f, "rb") as fp:
        assert qualify_candidate_anchor_fd(fp.fileno(), st.st_dev, st.st_ino, len(content), h, mtime_ns, ctime_ns + 9999) is False

def test_candidate_rejects_ctime_change_during_hash(tmp_path, monkeypatch):
    # Injected chmod during os.read modifies ctime
    # qualify_candidate_anchor_fd must detect ctime changed post-hash and return False
    pass
```
...
Create `app/quarantine/candidate.py` with descriptor-bound `fstat`, 1MiB SHA256 chunk streaming, and pre/post `mtime_ns` and `ctime_ns` checks.
...
- [ ] **Step 5: Focused regression & commit**
Run: `pytest tests/test_gate5g_g7_task3_candidate.py -v`
Commit: `git add app/quarantine/candidate.py tests/test_gate5g_g7_task3_candidate.py && git commit -m "feat(gate5g): implement descriptor-bound candidate qualification with ctime check"`
...
### Task 5: Capability Routing API & Direct Target Probing
...
- Produces: `resolve_mutation_capability(source_path, target_dir, quarantine_root) -> MutationCapability`.

- [ ] **Step 1: Write failing tests for capability resolver and direct directory probing**
```python
# tests/test_gate5g_g7_task5_capability.py
def test_probe_runs_on_target_directory_not_parent(tmp_path, monkeypatch):
    # Spy on dir_fd passed to _RENAME_AT_IMPL; assert it refers to target_dir, not parent
    pass
...
Create `app/quarantine/capability.py` implementing exact routing logic using descriptor `dir_fd`.
...
**Interfaces:**
- Consumes: `authoritative_anchor_path`, `renew_and_assert_worker_lease`, existing validation helpers.
...
- [x] **Gate3 Full Authority**: Both `mtime_ns` and `ctime_ns` checked pre-hash and post-hash across 1MiB SHA256 chunking. Qualification mismatch enters `conflict`.
...
- [x] **Durable Lease Fence**: Explicit committed protocol (`renew_and_assert_worker_lease(session_factory, worker_id)`) with zero write lock held during syscalls.
- [x] **Capability Routing**: Probes target directory directly via descriptor `dir_fd`; verifies device parity across source, target, and quarantine roots.
