# Gate5-G / G7 Transactional Mutation Implementation Plan (v1.1.1)
...
2. **Strict Frozen `tx_phase` Values**: Only the 10 frozen enum values are permitted:
   ...
   Zero invented or unapproved phases (e.g. no `candidate_created`, `source_captured_expected`, `restore_published`, `restore_view_captured`).
3. **Legacy Migration Rule**: Pre-existing rows retain their original `state` (`active`, `restored`, `purged`, etc.) and have `tx_phase = NULL` (or logical legacy classification). Never overwrite historical `state` to `'legacy'`.
4. **Zero Payload-Bearing Unlink**: In `COMPAT_TRANSACTIONAL` mode, unlinking anchors, captured sources, foreign objects, or unknown inodes is strictly forbidden.
5. **Durable & Short Lease Fence**:
   ```python
   with SessionLocal() as session:
       # BEGIN IMMEDIATE
       renew + assert current worker
       # COMMIT
   # No SQLite write transaction held here
   execute EXACTLY ONE payload-affecting filesystem syscall
   ```
   Repeated before each payload syscall. Zero SQLite write lock held across filesystem/FUSE calls.
6. **Capability Routing**:
   - `NATIVE_ATOMIC_NOREPLACE`: existing native atomic `rename_noreplace` path (zero transaction overhead).
   - `COMPAT_TRANSACTIONAL`: regular files use transaction engine.
   - `UNSUPPORTED`: `EOPNOTSUPP` fail-closed for COMPAT symlink/directory/special inodes.
   - `UNSUPPORTED`: fail-closed for inconclusive probe.
7. **Obsolete Fallback Decommissioning**: Legacy `rename_noreplace` raises `OSError(errno.EOPNOTSUPP)` when native renameat2 is unsupported; zero check-then-unlink fallback is executed.
8. **NAS Testbed Path Isolation**:
   - Validation must target sibling disposable root: `/mnt/zfpool/test_gate5g_candidate_<git_sha_short>/`.
   - Immutable preservation of old failure root `/mnt/zfpool/test_gate5g_compat_*` (never deleted or reused).
   - Fail-closed guards against production paths (`/volume1/*`, `/volume2/*`, `/data`, `/config`, `/etc`).
   - Sudo enforcement, `stat -f` zfuse check, `st_dev` equality, and sentinel verification.
9. **Codebase Mapping Veracity**: `execute_item` is defined in [`app/execution/executor.py`](file:///Users/Kerwin/MyProject/nas-file-center/app/execution/executor.py#L61) and imported by [`app/tasks/handlers.py`](file:///Users/Kerwin/MyProject/nas-file-center/app/tasks/handlers.py#L17).
10. **TDD Discipline**: Every Task must end GREEN against its focused regressions before committing.
...
| Component / File | Baseline Reality (`c32d077`) | Role in Implementation Plan v1.1.1 |
| :--- | :--- | :--- |
...
| [`app/fs_ops.py`](file:///Users/Kerwin/MyProject/nas-file-center/app/fs_ops.py#L150-L230) | `_probe_rename_noreplace_supported()` uses disposable files to test atomic rename. | Reused by capability resolver; returns `True`, `False`, or `None`. |
...
| [`app/batch_utilities/empty_dir_quarantine.py`](file:///Users/Kerwin/MyProject/nas-file-center/app/batch_utilities/empty_dir_quarantine.py#L90-L133) | `safe_open_parent_fd()` implements path traversal with `O_NOFOLLOW`. | Reused for ancestor path traversal and descriptor-relative operations. |
| [`app/tasks/recovery.py`](file:///Users/Kerwin/MyProject/nas-file-center/app/tasks/recovery.py#L29-L60) | `assert_active_worker_lease()` checks `TaskLock` row 1 with 30s timeout. | Extended with `renew_and_assert_worker_lease()` for short committed fences. |
...
### 2.1 Complete Gate3 Authority
In [`app/tasks/handlers.py::gather_reconcile_evidence`](file:///Users/Kerwin/MyProject/nas-file-center/app/tasks/handlers.py#L510-L535), Gate3 frozen authority consists of:
1. Object mode check: `stat.S_ISREG(st.st_mode)`.
2. Initial physical stat: `st_dev`, `st_ino`, `st_size`, `st_mtime_ns`, `st_ctime_ns`.
3. Streaming cryptographic hash: 1MiB chunked SHA256.
4. Post-hash stat snapshot: Re-verifying identical stat to detect concurrent mutation during hashing.

### 2.2 Descriptor-Bound Qualification Protocol
Candidate qualification executes strictly relative to an opened file descriptor to eliminate pathname re-opening races:
```python
def qualify_candidate_anchor_fd(
    ...
    if expected_mtime_ns is not None:
        actual_mtime_ns = getattr(st_before, "st_mtime_ns", int(st_before.st_mtime * 1e9))
...
### 2.3 Single Immutable Anchor Path Promotion
Candidate anchor is created directly at its permanent authoritative path:
`<quarantine_root>/.tx/entry-<id>/attempt-<gen>/anchor`.
Upon passing qualification, DB sets `authoritative_anchor_path` to this path and advances `tx_phase = 'authoritative_anchored'`. Zero rename, zero relink, zero unlinking.

---

## 3. Durable & Short Worker Lease Fence Protocol

### 3.1 Protocol Specification
To prevent holding SQLite write locks across slow FUSE syscalls while guaranteeing stale workers are fenced before executing filesystem mutations:
```python
def renew_and_assert_worker_lease(
    ...
    """
    Durable and short committed lease fence:
    1. Opens dedicated SessionLocal().
    2. Issues BEGIN IMMEDIATE.
    3. Asserts lock.locked, lock.owner == worker_id, age <= timeout_seconds.
    ...
    with SessionLocal() as session:
        ...
        now = utcnow()
        age = (now - lock.acquired_at.replace(tzinfo=timezone.utc)).total_seconds()
...
### 3.2 Caller Execution Pattern
Before each payload-affecting syscall:
```python
renew_and_assert_worker_lease(worker_id)
# execute EXACTLY ONE payload-affecting syscall (e.g. os.link, os.rename)
# repeat fence before next syscall
```

---

## 4. Capability Routing API
...
def resolve_mutation_capability(
    ...
    1. Probes target_dir using _probe_rename_noreplace_supported(target_dir).
    2. If True -> NATIVE_ATOMIC_NOREPLACE (native atomic renameat2 path).
    3. If False (e.g. zfuse returning EINVAL/ENOSYS/EOPNOTSUPP):
       - If source is regular file: return COMPAT_TRANSACTIONAL.
       - If source is symlink, directory, or special: return UNSUPPORTED (EOPNOTSUPP).
    4. If None (probe failed/inconclusive) -> return UNSUPPORTED (fail closed).
    """
    try:
    ...
    probe_result = _probe_rename_noreplace_supported(dir_path=target_dir)
...
| **Conflict Detected** | `conflict` | `conflict` | `<anchor_path>` or `NULL` | $G \ge 1$ | Collision detected; foreign occupant preserved in place. |
| **Failed Abort** | `failed` | `NULL` | `NULL` | $G \ge 1$ | Aborted before authority established; clean attempt cleaned. |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **1** | `preparing`, `preparing` | No attempt dir or no anchor in `attempt-<G>` | Stat attempt dir & anchor; verify absent. | Yes ($G+1$) if retrying | `preparing` / `preparing` (or `failed` if source missing) | NO payload mutation. Create `attempt-<G+1>` dir only. | Committed fence before `os.mkdir` | Retry or fail item |
| **2** | `preparing`, `preparing` | Candidate anchor exists in `attempt-<G>/anchor` | `open(O_RDONLY\|O_NOFOLLOW)`, run `qualify_candidate_anchor_fd()` against Gate3 identity | No if valid; Yes ($G+1$) if invalid | If valid: `authoritative_anchored` / `preparing`. If invalid: `preparing` / `preparing` | Zero unlink. If valid: DB update only. | Committed fence before DB promotion | Progress transaction |
| **3** | `preparing`, `preparing` | Generation $G$ committed in DB, but `attempt-<G>` directory missing on disk | Stat directory; confirms missing. | Yes ($G+1$) | `preparing` / `preparing` | `os.mkdir` for attempt $G+1$ under fence. | Committed fence before `os.mkdir` | Retry preparation |
| **4** | `preparing`, `authoritative_anchored` | Public quarantine path absent; anchor valid | Stat anchor (valid) and public path (absent). | No | `public_published` / `preparing` | YES: exactly ONE `os.link(anchor, public_path)`. | Committed fence before `os.link` | Progress to publication |
| **5** | `preparing`, `authoritative_anchored` | Public quarantine path present and matches anchor dev/ino | Stat public path & anchor; verify identical dev/ino (FS succeeded, DB lagged). | No | `public_published` / `preparing` | NO FS mutation needed. DB advance only. | Committed fence before DB commit | Progress to publication |
| **6** | `preparing`, `authoritative_anchored` | Public quarantine path present but occupied by foreign inode | Stat public path; dev/ino mismatch with anchor. | No | `conflict` / `conflict` | STRICTLY ZERO FS MUTATION. Foreign occupant preserved in place. | N/A (read-only) | `status='failed'`, record conflict |
| **7** | `preparing`, `public_published` | `attempt-<G>/captured_source` absent; source path present with matching inode | Stat capture slot (absent); stat source path (matches anchor). | No | `source_captured` / `preparing` -> `active` / `active` | YES: exactly ONE `os.rename(source, captured_source)`. | Committed fence before `os.rename` | Complete quarantine |
| **8** | `preparing`, `public_published` | `attempt-<G>/captured_source` present and matches anchor dev/ino | Stat capture slot; dev/ino match anchor (FS rename succeeded, DB lagged). | No | `source_captured` / `preparing` -> `active` / `active` | NO FS mutation needed. DB advance only. | Committed fence before DB commit | `status='succeeded'` |
| **9** | `preparing`, `public_published` | `attempt-<G>/captured_source` present but occupied by foreign inode | Stat capture slot; dev/ino mismatch with anchor. | No | `conflict` / `conflict` | STRICTLY ZERO FS MUTATION. Foreign occupant preserved in place. | N/A (read-only) | `status='failed'`, record conflict |
| **10**| `restoring`, `restoring` | Original destination absent; anchor valid | Stat original path (absent); stat anchor (valid). | No | `restored` / `restored` | YES: `os.link(anchor, original_path)`. | Committed fence before `os.link` | Progress restore |
| **11**| `restoring`, `restoring` | Original destination present and matches anchor dev/ino | Stat original path & anchor; verify matching dev/ino (FS link succeeded, DB lagged). | No | `restored` / `restored` | Optional: retire public view into slot if present. | Committed fence before view retirement | `status='succeeded'` |
| **12**| `restoring`, `restoring` | Original destination present but occupied by foreign inode / EEXIST | Stat original path; dev/ino mismatch (collision). | No | `conflict` / `conflict` | STRICTLY ZERO FS MUTATION. Foreign occupant preserved in place. | N/A (read-only) | `status='failed'`, record conflict |
| **13**| `restoring`, `restoring` | `attempt-<G>/captured_quarantine_view` present (expected or foreign) | Stat captured view slot. If original also matches: restore is complete. | No | `restored` / `restored` | Zero unlink. If original not yet restored: `os.link(anchor, orig)`. | Committed fence before `os.link` | `status='succeeded'` |
...
def test_race_r1_source_replaced_before_capture(tmp_path, db_session, worker_id):
...
def test_race_r2_source_replaced_after_earlier_verification_before_capture(tmp_path, db_session, worker_id):
...
def test_race_r3_source_recreated_immediately_after_capture(tmp_path, db_session, worker_id):
...
def test_race_r4_stale_worker_second_capture(tmp_path, db_session, worker_id):
...
def test_race_r5_two_generations_capture_different_occupants(tmp_path, db_session, worker_id):
...
def test_race_r6_public_destination_appears_before_publication(tmp_path, db_session, worker_id):
...
def test_race_r7_public_destination_replaced_after_publication(tmp_path, db_session, worker_id):
...
def test_race_r8_stale_worker_holds_old_tx_dir_fd(tmp_path, db_session, worker_id):
...
def test_race_r9_stale_worker_holds_source_parent_fd(tmp_path, db_session, worker_id):
...
def test_race_r10_current_worker_reconciles_while_old_worker_executes_one_fs_syscall(tmp_path, db_session, worker_id):
...
def test_race_r11_crash_after_anchor_creation_before_db_commit(tmp_path, db_session, worker_id):
    """R11: Crash occurs right after candidate anchor linked, before DB commit."""
    # DB is preparing, attempt-1/anchor exists on disk
    # Reconciler qualifies candidate anchor against Gate3 frozen identity
    # Assert: qualified, DB advanced to authoritative_anchored.

def test_race_r12_crash_after_capture_rename_before_db_commit(tmp_path, db_session, worker_id):
...
def test_race_r13_restore_original_destination_appears_concurrently(tmp_path, db_session, worker_id):
...
def test_race_r14_restore_public_view_replaced_before_retirement(tmp_path, db_session, worker_id):
    """R14: Public view replaced before restore retirement rename."""
    # Original successfully restored
    # Public view replaced with foreign occupant
    # Retirement moves foreign occupant into slot; detected as foreign; preserved in place; state='restored'.

def test_race_r15_private_attempt_contains_unknown_inode(tmp_path, db_session, worker_id):
...
def test_race_r16_multiple_attempt_directories_survive_restart(tmp_path, db_session, worker_id):
...
## 8. NAS Acceptance Path & Safety Guards

### 8.1 Sibling Disposable Root Specification
- **Allowed Validation Root**: `/mnt/zfpool/test_gate5g_candidate_<git_sha_short>/`.
- **Strictly Forbidden Paths**:
  - `/volume1/*`, `/volume2/*` (Production storage pools).
  - `/etc/*`, `/app/*`, `/data/*`, `/config/*` (Production OS & configuration).
  - `/mnt/zfpool/test_gate5g_compat_*` (**Old G7 failure evidence testbed — PRESERVED IMMUTABLY**).

### 8.2 Pre-Mutation Fail-Closed Guards
Before issuing any filesystem mutation on NAS:
1. **Parent Path Whitelist**: Target path must start with `/mnt/zfpool/test_gate5g_candidate_`.
...
4. **Evidence Preservation Guard**: Old evidence root `/mnt/zfpool/test_gate5g_compat_*` exists and is unmodified.
5. **Sentinel Verification**: `.sentinel` file verified in testbed root.
6. **Sudo Enforcement**: All NAS execution commands run under `sudo`.
...
### Task 2: Durable & Short Lease Fence Helper
...
**Interfaces:**
- Consumes: `SessionLocal`, `TaskLock`.
- Produces: `renew_and_assert_worker_lease(worker_id: str, timeout_seconds: float) -> None`.

- [ ] **Step 1: Write failing tests for committed lease fence**
```python
# tests/test_gate5g_g7_task2_lease_fence.py
def test_lease_fence_renewal_visible_across_sessions(db_engine):
    # Acquire lock for worker-1
    # Call renew_and_assert_worker_lease("worker-1")
    # In another SessionLocal(), assert acquired_at updated and lock NOT held
    pass

def test_lease_fence_zero_open_transaction_during_syscall(db_engine):
    # Call renew_and_assert_worker_lease("worker-1")
    # Assert no transaction open on db_engine
    pass

def test_lease_fence_rejects_stale_worker_post_takeover(db_engine):
    # Transfer lock to worker-2
    # with pytest.raises(JobLeaseLost):
    #     renew_and_assert_worker_lease("worker-1")
    pass
```
...
- [ ] **Step 3: Implement `renew_and_assert_worker_lease`**
Implement committed fence with `SessionLocal()`, `BEGIN IMMEDIATE`, and immediate commit.
...
Commit: `git add app/tasks/recovery.py tests/test_gate5g_g7_task2_lease_fence.py && git commit -m "feat(gate5g): implement durable committed worker lease fence"`

---

### Task 3: Gate3 SHA256 Authority & Descriptor-Bound Qualification
...
- [ ] **Step 1: Write failing test for descriptor-bound qualification**
```python
# tests/test_gate5g_g7_task3_candidate.py
def test_qualify_candidate_anchor_fd_success(tmp_path):
    ...
    with open(f, "rb") as fp:
        assert qualify_candidate_anchor_fd(fp.fileno(), st.st_dev, st.st_ino, len(content), h) is True

def test_qualify_candidate_anchor_fd_tampered_hash_fails(tmp_path):
    f = tmp_path / "candidate.txt"
    f.write_bytes(b"TAMPERED")
    st = os.stat(f)
    with open(f, "rb") as fp:
        assert qualify_candidate_anchor_fd(fp.fileno(), st.st_dev, st.st_ino, 8, "wrong_hash") is False
```
...
- [ ] **Step 3: Implement qualification algorithm**
Create `app/quarantine/candidate.py` with descriptor-bound `fstat` and 1MiB SHA256 streaming check.
...
- [ ] **Step 5: Focused regression & commit**
Run: `pytest tests/test_gate5g_g7_task3_candidate.py -v`
Commit: `git add app/quarantine/candidate.py tests/test_gate5g_g7_task3_candidate.py && git commit -m "feat(gate5g): implement descriptor-bound candidate anchor qualification"`
...
**Interfaces:**
- Consumes: `SessionLocal`, `QuarantineEntry`.
- Produces: `allocate_next_generation(entry_id, worker_id) -> tuple[int, Path]`.

- [ ] **Step 1: Write failing test for monotonic generation allocation**
```python
# tests/test_gate5g_g7_task4_allocator.py
def test_allocate_next_generation_monotonic(db_session, tmp_path):
...
Implement allocation under SQLite `BEGIN IMMEDIATE` committed before `os.mkdir`.
...
- [ ] **Step 5: Focused regression & commit**
Run: `pytest tests/test_gate5g_g7_task4_allocator.py -v`
Commit: `git add app/quarantine/tx_allocator.py tests/test_gate5g_g7_task4_allocator.py && git commit -m "feat(gate5g): implement monotonic generation allocation"`

---

### Task 5: Capability Routing API (`resolve_mutation_capability`)
...
**Interfaces:**
- Consumes: `_probe_rename_noreplace_supported`.
- Produces: `resolve_mutation_capability(source_path, target_dir) -> MutationCapability`.

- [ ] **Step 1: Write failing test for capability resolver**
```python
# tests/test_gate5g_g7_task5_capability.py
def test_resolve_native_regular_file(tmp_path, monkeypatch):
    # Mock probe returning True
    # Assert returns MutationCapability.NATIVE_ATOMIC_NOREPLACE
    pass

def test_resolve_compat_regular_file(tmp_path, monkeypatch):
    # Mock probe returning False
    # Assert returns MutationCapability.COMPAT_TRANSACTIONAL
    pass

def test_resolve_compat_symlink_unsupported(tmp_path, monkeypatch):
    # Mock probe returning False, source is symlink
...
Create `app/quarantine/capability.py` implementing exact routing logic.
...
- [ ] **Step 5: Focused regression & commit**
Run: `pytest tests/test_gate5g_g7_task5_capability.py -v`
Commit: `git add app/quarantine/capability.py tests/test_gate5g_g7_task5_capability.py && git commit -m "feat(gate5g): implement mutation capability resolver"`
...
- [ ] **Step 1: Update tests in `test_gate5g_g7_hotfix1_fs_ops.py`**
...
def test_transactional_quarantine_success(tmp_path, db_session, worker_id):
...
Implement 5-step mutation flow with committed lease fences before each syscall.
...
### Task 8: Transactional Restore Engine & Foreign Inode Protection

**Files:**
- Create: `app/quarantine/restore.py`
...
**Interfaces:**
- Consumes: `authoritative_anchor_path`, `renew_and_assert_worker_lease`.
- Produces: `execute_transactional_restore(entry, worker_id) -> None`.

- [ ] **Step 1: Write failing tests for restore engine**
```python
# tests/test_gate5g_g7_task8_restore.py
def test_restore_preserves_foreign_inode_on_destination_collision(tmp_path, db_session, worker_id):
    # Setup active compat entry
    # Place foreign file at original destination
    # Execute restore
    # Assert: state='conflict', foreign file untouched, payload safe in anchor.
    pass
```
...
Implement restore flow with hard-link creation, public view retirement, and collision detection.
...
- [ ] **Step 5: Focused regression & commit**
Run: `pytest tests/test_gate5g_g7_task8_restore.py -v`
Commit: `git add app/quarantine/restore.py app/service.py tests/test_gate5g_g7_task8_restore.py && git commit -m "feat(gate5g): implement transactional restore with foreign inode protection"`
...
def test_api_boot_defers_transactional_entry(db_session, monkeypatch):
...
def test_reconcile_preparing_with_candidate_anchor(tmp_path, db_session, worker_id):
    # Simulate Variant 2
    pass

def test_reconcile_authoritative_anchored_public_present_matches(tmp_path, db_session, worker_id):
    # Simulate Variant 5
    pass

def test_reconcile_public_published_captured_source_present_foreign(tmp_path, db_session, worker_id):
    # Simulate Variant 9
    pass
```
...
def test_purge_refuses_active_compat_entry(db_session):
...
- [x] **13 DB-Lag Variants**: Exhaustive matrix defines DB/FS facts, actions, generation rules, and outcomes.
- [x] **Durable Lease Fence**: Explicit committed protocol (`BEGIN IMMEDIATE` -> renew -> `COMMIT`) with zero write lock held during syscalls.
- [x] **Capability Routing**: Explicit resolver routing native regular files to atomic rename, compat regular files to transaction engine, and failing closed on unsupported.
- [x] **Obsolete Test Expectations**: `tests/test_gate5g_g7_hotfix1_fs_ops.py` updated to expect `EOPNOTSUPP` and zero link+unlink fallback.
- [x] **R1–R16 Test Mapping**: Dedicated file `tests/test_gate5g_g7_races.py` with 16 explicit test functions.
- [x] **NAS Path Safety**: Validation root isolated under sibling disposable `/mnt/zfpool/test_gate5g_candidate_<sha>/`; old evidence preserved immutably.
- [x] **Codebase Mapping**: Correctly references `app/execution/executor.py::execute_item`.
